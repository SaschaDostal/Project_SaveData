    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        if (filter == Filter.ALL) {
            if (columnIndex == 0) {
                ...
                return DataHandler.getElements("GStatus")[rowIndex];
            } else {
                return "error";
            }
        } else if (filter == Filter.FINISHED) {
            return DataHandler.getFinishedElements("Game", true)[rowIndex][columnIndex];
        } else if (filter == Filter.PENDING) {
            return DataHandler.getFinishedElements("Game", false)[rowIndex][columnIndex];
        } else {
            return null;
        }
    }
